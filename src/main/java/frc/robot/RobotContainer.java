...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        NamedCommands.registerCommand("coral",
                elevator.ElevatorClawMove(Constants.Elevator.State.REEF_4)
                        ...
                        .andThen(Commands.defer(() -> Commands.waitSeconds(0.5)
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                ...
                // drive.withVelocityX(Utility.quadraticCurve(-m_driverJoystick.getLeftY()) *
                // MaxSpeed)// Drive forward with negative Y (forward)
                // .withVelocityY(Utility.quadraticCurve(-m_driverJoystick.getLeftX() )*
                // MaxSpeed)// Drive left with negative X (left)
                // .withRotationalRate(Utility.quadraticCurve(-m_driverJoystick.getRightX()) *
                ...
                drive.withVelocityX((-m_driverJoystick.getLeftY()) * MaxSpeed)// Drive forward with negative Y (forward)
                        ...
                        .withRotationalRate((-m_driverJoystick.getRightX()) * MaxAngularRate)// Drive counterclockwise
                                                                                             // with negative X (left)
                ));

        // m_driverJoystick.a().whileTrue(drivetrain.applyRequest(() -> brake));
        // m_driverJoystick.b().whileTrue(drivetrain.applyRequest(() ->
        // point.withModuleDirection(new Rotation2d(-m_driverJoystick.getLeftY(),
